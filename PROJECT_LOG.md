...
- Added a TUI-like context usage indicator above the web composer showing estimated used tokens vs. the active model context window, including percentage and a small progress bar. The indicator is sourced from pi `AgentSession.getContextUsage()`, updates after completed turns, and the fake-agent harness now asserts it via a `context-usage` scenario.
