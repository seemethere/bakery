...
- Adjacent completed non-image tool rows now render as a tighter visual activity group, and snapshot hydration drops assistant messages that only repeat tool-call metadata while carrying their labels onto the corresponding tool result row.
