...
Latest: `bun run check` and focused `bun scripts/ui-harness.ts --scenario artifact-path-formats` pass after adding inline/fenced/long local artifact regression coverage; focused artifacts at `test-results/ui-harness/artifact-path-formats-2026-04-26T20-49-16-880Z`. A previous focused `bun scripts/ui-harness.ts --scenario repeated-image-artifact-paths` passed after adding repeated local artifact regression coverage with artifacts at `test-results/ui-harness/repeated-image-artifact-paths-2026-04-26T20-47-33-061Z`. A previous focused `bun scripts/ui-harness.ts --scenario image-artifact-paths` passed after removing cross-message local image artifact suppression with artifacts at `test-results/ui-harness/image-artifact-paths-2026-04-26T20-44-56-887Z`. A previous focused `bun scripts/ui-harness.ts --scenario narrow-tool-stream` passed after compact successful-tool summary/background tuning with artifacts at `test-results/ui-harness/narrow-tool-stream-2026-04-26T20-37-00-981Z`. Latest full harness scenario set before that: `all` (`streaming-responsiveness`, `transcript-scroll-stability`, `inspector-preview`, `slash-commands`, `tree-fork-navigation`, `reconnect-controller`, `controller-handoff-edges`, `reconnect-draft`, `backend-restart`, `narrow-tool-stream`, `file-autocomplete`, `image-attachments`, `image-artifact-paths`, `model-thinking`); artifacts at `test-results/ui-harness/all-2026-04-26T20-32-25-593Z`. A focused direct run, `bun scripts/ui-harness.ts --scenario image-artifact-paths`, also passed with artifacts at `test-results/ui-harness/image-artifact-paths-2026-04-26T20-32-14-492Z`. Earlier attempted `bun run test:web-perf -- --scenario streaming-responsiveness` runs invoked the full `all` set and exposed sidebar/harness regressions before the final fix. `bun scripts/ui-harness.ts --scenario manual --keep` opens and seeds the manual headed harness successfully; terminating it via SIGINT prints the artifact/temp paths before shutdown. On a fresh machine, run `bun x playwright install chromium` once if Playwright reports a missing browser.

## Next priorities

1. Continue transcript noise reduction: consider deeper grouping of tool call/result pairs, terminal-output viewport sizing, local image artifact thumbnail sizing, and queue chips after real-session dogfooding.
