...
- Added a TUI-like context availability notice above the web composer, populated from pi session resources (context files, skills, extensions, and prompt templates) and refreshed after `/reload`; the fake-agent harness now asserts the notice via a `context-availability` scenario.
