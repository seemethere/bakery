...
export async function runBundledExtensionCommand(name: string, args: string): Promise<ExtensionCommandResult | undefined> {
  const entry = bundledExtensionCommandsByName.get(name);
  if (!entry) return undefined;
  return await entry.command.handler({ extensionId: entry.extension.id }, args);
}
