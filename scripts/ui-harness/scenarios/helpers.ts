...
export async function collectMetrics(page: Page): Promise<Record<string, unknown>> {
  return await page.evaluate(() => {
    ...
    return {
      ...
      transcriptRows: document.querySelectorAll("[data-transcript-id]").length,
      transcriptToolRows: document.querySelectorAll(".message.tool").length,
      transcriptAssistantRows: document.querySelectorAll('[data-transcript-kind="assistant"]').length,
      transcriptArtifactImages: document.querySelectorAll(".artifact-image img").length,
      transcriptDomNodes: transcript ? transcript.querySelectorAll("*").length : 0,
