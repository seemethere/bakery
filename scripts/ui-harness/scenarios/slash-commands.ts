...
export async function runSlashCommands(page: Page): Promise<Record<string, unknown>> {
  ...
  await planCard.locator('[data-plan-outcome="accepted"]', { hasText: "Accepted" }).waitFor({ timeout: 5_000 });
  await planCard.locator('[data-plan-action="accept"]').waitFor({ state: "detached", timeout: 5_000 });
  await page.waitForFunction(() => (document.querySelector<HTMLTextAreaElement>("#prompt")?.value ?? "") === "", null, { timeout: 5_000 });
  ...
  await page.locator('.plan-details-dialog [data-plan-outcome="accepted"]', { hasText: "Accepted" }).waitFor({ timeout: 5_000 });
