...
async function runSlashCommands(page: Page): Promise<Record<string, unknown>> {
  ...
  await page.locator("#planFeedback").fill("make the action surface take over the composer");
  await planActions.locator('[data-plan-action="feedback"]').click();
  ...
  const feedbackSubmitted = await page.locator(".message.user", { hasText: "Feedback on the plan: make the action surface take over the composer" }).waitFor({ timeout: 1_500 }).then(() => true).catch(() => false);
  if (!feedbackSubmitted) await page.locator(".queue-pill", { hasText: "Feedback on the plan" }).waitFor({ timeout: 5_000 });
  await page.locator(".status.idle").waitFor({ timeout: 10_000 });
  await ensureSidebarSettingsVisible(page);
  await page.locator(".session-card.active .session-snippet", { hasText: "Feedback on the plan" }).waitFor({ timeout: 5_000 });
  if (await page.locator("#sessionSidebarBackdrop").isVisible().catch(() => false)) await page.locator("#sessionSidebarBackdrop").click();
